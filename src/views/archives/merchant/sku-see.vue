    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <h4 class="col-lg-12 table-detail">
                <span>商品编号： {{code}}</span> <span>商品名称：{{name}}</span>
            </h4>
            <div class="col-lg-12">
            ...
            <div class="operate-form-buttons col-lg-12">
                <a class="btn btn-default btn-back-list" href="#!/sku/index">返回列表<i class="icon-arrow-right iconfont"></i></a>
            </div>
        </div>
    </div>
</template>
    ...
    module.exports = {
        ...
        route: {
            data: function (transition) {
                // 当前路由的query参数赋值到页面的数据模型
                this.BarcodeID = transition.to.query.id;
                this.name = transition.to.query.name;
                this.code = transition.to.query.code;
                this.sku = transition.to.query.sku;
        ...
        methods: {
            getSkuList: function () {
                ...
                require('../../../js/servers/servers').postAjax({
                    url: '/BasBarcode/ErpQueryInventory',
                    data: {
                        BarcodeID: relf.BarcodeID
                    },
