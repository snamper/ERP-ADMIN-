    ...
    <div class="query-form">
        ...
        <div id="query-detail" class="form-inline query-detail collapse in" aria-expanded="true">
            <div class="row">
                ...
                <div class="form-group col-lg-3 col-md-4 col-sm-6">
                    <label>供应商</label>
                    <select class="form-control" v-model="query.MerchantID">
                        <option value="">请选择</option>
                        <option v-for="option in joinType" value="{{option.MerchantID}}">{{option.Name}}</option>
                    </select>
                </div>
            </div>
        </div>
    ...
    module.exports = {
        ...
        watch: {
            'checkedGroups': function (newValue) {
                this.refGoodsId = newValue;
            },
            'query.MerchantID': function (newValue) {
        ...
        methods: {
            ...
            getjoinSupplier: function () {
                var relf = this;

                servers.getBaseData('MerchantList',false,function(val){
                    relf.joinType = val;
                })
            },
            ...
            search: function() {
                if(this.query.MerchantID == ""){
                    return this.$dispatch('transmit', 'tip', {contentText: '请先选择一个供应商!'});
                }else{
                    this.erpQueryGoods();
                }
