<template>
    ...
    <div class="printMain" v-bind:style="pattern.paper.style" v-for="pack in packLists" :class="{'page-next': $index < packLists.length -1 }">
        ...
        <div class="print-fields"  v-for="editField in pattern.editField  | orderBy 'type'">
            <div class="formTable" v-if="editField.type==3">
                <table :style="pattern.detailsList.style">
                    ...
                    <tbody>
                    <tr v-for="packDetail in pack.packDetail">
                        <td v-for="list in pattern.detailsList.fields" v-bind:style="list.style">{{$interpolate(list.content) | nullToString  | joinToParent pack list.content}}</td>
                    </tr>
                    </tbody>
                </table>
            </div>
...
//                },1000);
            ...
            autoPrint: function () {
                ...
                var fileName = 'pack' + year + month + day + hours + minutes + seconds + '.html';
                a.download = fileName;
                a.style.opacity = 0;
...
//                var printData = '<data>' + document.querySelector('html').innerHTML + '</data>';
        ...
        route: {
            data: function (transition) {
                ...
                self.getDataUrl='/Print/ErpQueryPackSheetData'
                if(flag &&flag =='out'){
                    this.getDataUrl="/Print/ErpQueryPackStockSheetData"
                }
