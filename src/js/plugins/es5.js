...
(function () {
	...
	var DatePicker = function () {
        function DatePicker(el, params) {
            ...
            // 将时间插件定位至body元素内
            $('.w-content').eq(0).append(this.$el);
            var offset = el.offset();
            var left = offset.left - $('.sidebar-container').eq(0).outerWidth();
            var top = offset.top + el.outerHeight() - 41;
            if ($('.navright').offset().left) {
                left -= 150;
            }
            this.$el.css({
                zIndex: 98,
        ...
        _createClass(DatePicker, [{
            ...
            value: function nextDate(event) {
                ...
                this.params.onShow(this.dateStart.format(this.params.format), this.dateEnd.format(this.params.format));
                event.stopPropagation();
            }
        }, {
            key: 'prevDate',
            ...
            value: function initEvents() {
                ...
                $(document).on('click', function (event) {
                	if (event.target.id !== $(_this.el).attr('id')) {
